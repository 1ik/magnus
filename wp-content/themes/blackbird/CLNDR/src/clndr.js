...
;(function ( $, window, document, undefined ) {
  ...
  Clndr.prototype.bindEvents = function() {
    ...
    var xhr = '';
    $container.on('mouseenter' , '.'+this.options.targets.day, function(event){
        var target = self.buildTargetObject(event.currentTarget, true);
        window.date_req = target.date._i;
        // xhr = $.get('course-agenda?date='+target.date._i, function(data) {
        //     if(data.length > 0) {
        //         $('.tooltip').
        //     }
        // });
    ...
    $container.on('mouseleave' , '.'+this.options.targets.day, function(event){
        //alert('leave');
        if(xhr != '') {
          xhr.abort();
        }
